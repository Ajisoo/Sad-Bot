...
async def cmd_ga_refresh(bot, message, _args):
    ...
    await message.channel.send(
        "Refreshing content. Expect failing commands until done."
    )

    r = requests.get(GA_WEBSITE)
    soup = BeautifulSoup(r.content, "html.parser")

    ability_list = soup.find("div", {"class": "ability-list"}).find_all(
        "a", {"class": "ability-list__item"}
    )
    counter = 0
    for ability in ability_list:
        champ_name = ability.find("img", {"class": "ability-list__item__champ"})[
            "champ"
        ].replace("\n", " ")
        image = ability.find("img", {"class": "ability-list__item__pic"})[
            "data-original"
        ]
        ability_name = (
            str(ability.find("span", {"class": "ability-list__item__name"}).contents[0])
            .strip()
            .replace("\n", " ")
        )
        ability_key = ability.find(
            "div", {"class": "ability-list__item__keybind"}
        ).text.replace("\n", " ")
...
async def cmd_guess(bot, message, args):
    ...
    if bot.g_answer == guess_arg:
        ...
        commands.leaderboard_util.update_leaderboards_file(
            message.author.id, bot.guess_type
        )
...
async def cmd_ga_start(bot, message, args):
    ...
    if len(args) > 0 and (
        args[0].lower() == "c"
        or args[0].lower() == "champ"
        or args[0].lower() == "champion"
    ):
        info_file.readline()
        ...
        bot.g_answer = re.sub(r"[^a-z0-9]", "", bot.g_answer_raw.lower())
        await message.channel.send(
            "Guess the key (P, Q, W, E, R) this ability belongs to!"
        )
    else:
        bot.g_answer_raw = info_file.readline()
        bot.g_answer = re.sub(r"[^a-z0-9]", "", bot.g_answer_raw.lower())
    ...
    await message.channel.send(
        file=(discord.File(os.path.join(GA_FOLDER, f"{rand}img.png")))
    )
...
async def cmd_gs_refresh(bot, message, _args):
    ...
    get_aliases = GET_remote_file(
        cdragon_champsummaries_url, RS_ID_TO_ALIAS_MAPPINGS_FILE, message
    )
    ...
    for fname in os.listdir(TENTH_ANNIVERSARY_SKINS_FOLDER):
        shutil.copy2(
            os.path.join(TENTH_ANNIVERSARY_SKINS_FOLDER, fname), CHAMP_SPLASH_FOLDER
        )
    ...
    with open(SKINS_DATAFILE, "r") as f, open(TENTH_ANNIVERSARY_SKINS_JSON, "r") as f2:
        ...
        for k, v in skin_data.items():
            ...
            new_skin_data[k] = {
                key: v[key] for key in ["id", "name", "description", "rarity"]
            }
    ...
    with open(RS_ID_TO_ALIAS_MAPPINGS_FILE, "r+", encoding="utf-8") as f:
        ...
        aliases = {
            str(summary_data[i]["id"]): summary_data[i]["alias"]
...
async def cmd_gs_start(bot, message, _args):
    ...
    if latest_version is None:
        try:
            ...
            await message.channel.send(
                "Latest version file doesn't exist, please refresh"
            )
            return
    ...
    if not os.path.exists(champ_data_folder):
        await message.channel.send(
            "Champ data folder doesn't exist, ask admin to refresh"
        )
        return

    json_data = None
    with open(
        os.path.join(champ_data_folder, "champion", f"{champ_name}.json"),
        "r",
        encoding="utf-8",
    ) as f:
...
async def _umq_try_join_vc(_bot, message):
    ...
    try:
        ...
        if not voice:
            await message.channel.send(
                "You must be in a voice channel to start the quiz!"
            )
            return None
...
async def cmd_umq_start(bot, message, _args):
    ...
    vc.play(
        discord.FFmpegPCMAudio(
            executable="./ffmpeg.exe", source=song_path, options="-ss " + ts
        )
    )
...
async def upload_splashes_to_burner_channel(burner_channel: discord.TextChannel):
    ...
    with open(SKINS_DATAFILE, "r") as f:
        ...
        for k, v in skins_data.items():
            if (
                k + "A" in image_links_mapping
            ):  # already have it, but find a better way than k + 'A'
                continue
