...
async def on_message(message: discord.Message):
    ...
    if message.author.status == discord.Status.dnd:
        try:
            await message.add_reaction(
                await message.guild.fetch_emoji(634171978420322335)
            )
        except:
            pass
    ...
    if message.guild.id == LOUNGE_GUILD_ID and JAIL_ROLE in [
        y.id for y in message.author.roles
    ]:
        try:
            if message.guild.id == TEST_SERVER_GUILD_ID:  # test server
                await message.add_reaction(
                    await message.guild.fetch_emoji(834785299292487701)
                )
            else:
                await message.add_reaction(
                    await message.guild.fetch_emoji(831228255063244831)
                )  # shork reacc for jail
    ...
    if command == "essaymode":
        if message.author.id in ESSAY_MODE_ADMINS:
            ...
            await message.channel.send(
                "Please apply for privileges to use this command."
            )

    if (
        len(message.content) < bot.essay_mode_minimum
        and bot.essay_mode_on
        and bot.essay_mode_minimum > 0
    ):
        try:
    ...
    if command == "ga_my_score":
        await message.channel.send(
            leaderboard_util.get_my_points(message.author.id, GA_LEADERBOARD_ID)
        )

    if command == "gs_my_score":
        await message.channel.send(
            leaderboard_util.get_my_points(message.author.id, GS_LEADERBOARD_ID)
        )

    if command == ["gum_my_score", "umq_my_score"]:
        await message.channel.send(
            leaderboard_util.get_my_points(message.author.id, GUM_LEADERBOARD_ID)
        )
    ...
    if command in ["playmp3"]:
        ...
        vc.play(
            discord.FFmpegPCMAudio(
                executable=RES_FFMPEG,
                source=os.path.join(CONTENT_FOLDER, args[0] + ".mp3"),
            )
        )
        while vc.is_playing():
    ...
    if command in ["ts", "trade_splash"]:
        if len(args) in [1, 3]:
            ...
            await message.channel.send(
                (
                    "Please follow one of the trading formats:\n"
                    "Trade initiation:\n"
