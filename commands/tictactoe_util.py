...
def check_if_win(game, char):
    if game[0] == char and game[1] == char and game[2] == char:
        return True
    if game[3] == char and game[4] == char and game[5] == char:
        return True
    if game[6] == char and game[7] == char and game[8] == char:
        return True

    if game[0] == char and game[3] == char and game[6] == char:
        return True
    if game[1] == char and game[4] == char and game[7] == char:
        return True
    if game[2] == char and game[5] == char and game[8] == char:
        return True

    if game[0] == char and game[4] == char and game[8] == char:
        return True
    if game[2] == char and game[4] == char and game[6] == char:
        return True
...
async def user_input(data, player_id, message, reaction):
    if messages.get(player_id) == message:
        ...
        if check_if_win(data.data, "o"):
            await message.edit(
                embed=generate_embed(message.embeds[0].title[0:-23], data)
            )
...
def generate_embed(name, data):
    if check_if_win(data.data, "o"):
        embed = discord.Embed(
            title=name + " won!",
            description="",
            color=discord.Color.from_rgb(0, 99, 255),
        )
        ...
        embed = discord.Embed(
            title=name + "'s game of Tic Tac Toe!",
            description="",
            color=discord.Color.from_rgb(0, 99, 255),
        )
    for i in range(9):
