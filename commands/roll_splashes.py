...
def create_user_data_files():
    ...
    if (
        not os.path.exists(RS_SKIN_NAME_TO_ID_MAPPINGS_FILE)
        and os.path.exists(SKINS_DATAFILE)
        and os.path.exists(TENTH_ANNIVERSARY_SKINS_JSON)
    ):
        ...
        with open(RS_SKIN_NAME_TO_ID_MAPPINGS_FILE, "w+") as f, open(
            SKINS_DATAFILE, "r"
        ) as f2:
...
async def force_roll(bot, message, id_with_piece, user_to_roll_for):
    ...
    if m is not None:
        ...
        await cmd_splash_roll(
            bot,
            message,
            forced_id=id_with_piece[:-1],
            forced_piece=id_with_piece[-1].upper(),
            roll_receiver_id=user_to_roll_for,
        )
    else:
        await message.channel.send(
            "Please format `$rs <skin_id>[A|B|C|D] <@roll_recipient>`"
        )


async def cmd_splash_roll(
    bot, message, forced_id=None, forced_piece=None, roll_receiver_id=None
):
    if not os.path.exists(CHAMP_SPLASH_FOLDER):
        ...
        return
    if not os.path.exists(RS_ID_TO_ALIAS_MAPPINGS_FILE) or not os.path.exists(
        RARITY_DIST_FILE
    ):
        print("RARITY DIST OR CHAMPION SUMMARY DOESN'T EXIST, PLEASE REFRESH")
        await message.channel.send(
            "Champs can't be rolled right now. Please ask an admin to refresh the champ splashes!"
        )
        return
    ...
    if forced_id is None:
        ...
        if percentages is None or rolls is None:
            with open(RARITY_DIST_FILE, "r") as f:
                ...
                loot_pools = [
                    (v["percentage"], v["rolls"]) for v in RARITY_DIST.values()
                ]
    ...
    with open(SKINS_DATAFILE, "r") as f:
        ...
        full_skin_name, champ_description = [
            champ_data["name"],
            champ_data["description"],
        ]
    ...
    embed = discord.Embed(
        ...
        url=ddragon_baseurl + chosen_splash,
...
async def cmd_splash_list(bot, message, args, client):
    if not os.path.exists(SPLASH_HAREM_FILE):
        ...
        await message.channel.send(
            "Splash harems aren't available right now, please contact an admin."
        )
        return
    if not os.path.exists(SKINS_DATAFILE):
        print("SKINS FILE DOESN'T EXIST, RESTART BOT")
        await message.channel.send(
            "Skin info isn't available right now, please contact an admin."
        )
        return
    ...
    if len(champs) == 0:
        ...
        if sort_method == "name":
            ...
            sorted_champs = sorted(
                champs, key=lambda x: rarity_sort[champs[x]["rarity"]]
            )
        else:
        ...
        for k in sorted_champs:
            ...
            if all(pieces_count.values()) and not show_number:
                champs_list.append(
                    f'#**{k}**: {champs[k]["name"]} (**Complete**) {rarity_symbol(rarity, bot)}'
                )
            else:
                champs_list.append(
                    f'#**{k}**: {champs[k]["name"]} (Pieces: {", ".join(pieces)}) {rarity_symbol(rarity, bot)}'
                )

        # Make scrollable embed
        chunks = [
            champs_list[i : i + champs_per_page]
            for i in range(0, len(champs_list), champs_per_page)
        ]
        champs_desc = "\n".join(champs_list[:champs_per_page])

        embed = discord.Embed(
            title=f"{harem_owner}'s champs", description=champs_desc
                ...
                if skin_id in user_harem:
                    msgs.append(
                        message.channel.send(
                            "Divorced %s!"
                            % (user_harem[skin_id]["name"] + " (Piece " + l + ")")
                        )
                    )
...
async def trade_splashes(bot, message, args):
    ...
    if len(args) == 3:
        ...
        if bot.active_trades.get(tradee_id, None) is not None:
            await message.channel.send(
                "Active trade in progress with that user already! Try again later."
            )
            return
        ...
        await message.channel.send(
            (
                f"<@{tradee_id}>, {trader_offer} (Piece {trader_piece}) is being"
        ...
        if response in ["y", "yes"]:
            with open(SPLASH_HAREM_FILE, "r+") as f:
                harems = json.load(f)
                trader_skin_id, trader_piece = [
                    trade["trader_offer"][:-1],
                    trade["trader_offer"][-1],
                ]
                their_offer = (
                    harems.get(trade["trader_id"], {})
                    .get(trader_skin_id, {})
                    .get("pieces", {})
                    .get(trader_piece)
                )
                if their_offer == 0 or their_offer is None:
                    await message.channel.send(
                        "Trader doesn't have their offer, please no bamboozle >:(. Cancelling trade."
                    )
                    del bot.active_trades[user_id]
                    return

                tradee_skin_id, tradee_piece = [
                    trade["tradee_offer"][:-1],
                    trade["tradee_offer"][-1],
                ]
                your_offer = (
                    harems.get(user_id, {})
                ...
                if your_offer == 0 or your_offer is None:
                    await message.channel.send(
                        "YOU don't have your offer, bamboozling is strictly prohibited. Trade cancelled."
                    )
                    del bot.active_trades[user_id]
                    return
                ...
                if tradee_skin_id in their_harem:
                    ...
                    their_harem[tradee_skin_id] = get_starting_pieces(
                        tradee_skin_name, tradee_piece, tradee_rarity
                    )

                if trader_skin_id in your_harem:
                    your_harem[trader_skin_id]["pieces"][trader_piece] += 1
                else:
                    your_harem[trader_skin_id] = get_starting_pieces(
                        trader_skin_name, trader_piece, trader_rarity
                    )
...
def create_progress_embed(
    skin_name, description, rarity, splash_fname, pieces_counts, im, bot
):
    ...
    embed = discord.Embed(
        title=title,
        url=ddragon_baseurl + splash_fname,
        description=desc,
        color=rarity_colors[rarity],
    ).set_image(url=attachment_prefix + CROPPED_IMAGE_FNAME)
    f = discord.File(CROPPED_IMAGE_FNAME)
