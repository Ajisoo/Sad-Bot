...
stars = {
    "5": ":star2: :star2: :star2: :star2: :star2:",
...
async def cmd_show_color(bot, message, args):
    ...
    if (
        len(args) > 0
        and (args[0][0] == "-" or args[0][0].isdigit())
        and (len(args[0]) == 1 or args[0][1:].isdigit())
    ):
        val = int(args[0])
    ...
    img = Image.new(
        "RGB",
        (color_size[data[0]], color_size[data[0]]),
        (max(0, int(data[1])), max(0, int(data[2])), max(0, int(data[3]))),
    )
...
async def cmd_list_color(bot, message, args):
    ...
    for index, thing in data:
        _thing = thing.split("|")
        string += (
            f"[{index}] {stars[_thing[0]]} RGB {_thing[1]} {_thing[2]} {_thing[3]}\n"
        )
...
async def first_time_setup(bot):
    if bot.start_date is None:
        if os.path.exists("content/roll_color/roll_start_date.txt"):
            with open("content/roll_color/roll_start_date.txt", "r") as f:
                bot.start_date = datetime.datetime.strptime(
                    f.readline()[:-1], "%Y-%m-%d %H:%M:%S"
                )
        else:
            bot.start_date = datetime.datetime.strptime(
                "2021-01-17 00:00:00", "%Y-%m-%d %H:%M:%S"
            )
    ...
    if (datetime.datetime.now() - bot.start_date).days >= 1:
        ...
        bot.start_date = bot.start_date + datetime.timedelta(
            days=int((datetime.datetime.now() - bot.start_date).days / 1)
        )
        with open("content/roll_color/roll_start_date.txt", "w+") as f:
...
async def cmd_roll(bot, message, _args):
    if (
        message.author.id in bot.roll_user_data.keys()
        and bot.roll_user_data[message.author.id][0] == "1"
    ):
        ...
        if data[0] == "0":
            embed = discord.Embed(
                title=f"{message.author.name}, you have already rolled!",
        ...
        if num < 0.05:
            ...
            r, g, b = (0, 0, 0)
            while (
                r in [0, 64, 128, 192, 255]
                and g in [0, 64, 128, 192, 255]
                and b in [0, 64, 128, 192, 255]
            ):
        ...
        base_embed = discord.Embed(
            title=f"{message.author.name}, you rolled :question: :question: :question:"
        )
        r_embed = discord.Embed(
            title=f"{message.author.name}, you rolled ***{r}*** :question: :question:"
        )
        g_embed = discord.Embed(
            title=f"{message.author.name}, you rolled ***{r}*** ***{g}*** :question:"
        )
        b_embed = discord.Embed(
            title=f"{message.author.name}, you rolled ***{r}*** ***{g}*** ***{b}***"
        )

        if message.author.id not in bot.roll_user_data.keys():
            bot.roll_user_data[message.author.id] = ("0", [])
        bot.roll_user_data[message.author.id] = (
            "1",
            bot.roll_user_data[message.author.id][1],
        )
        bot.roll_user_data[message.author.id][1].insert(
            0, f"{data[0]}|{data[1]}|{data[2]}|{data[3]}"
        )
        await update_file(bot.roll_user_data)
        ...
        if data[0] == "0":
            embed = discord.Embed(
                title=f"{message.author.name}, you rolled a 0 star color! Impressive!",
                description="View it on the left.",
            )
        else:
            embed = discord.Embed(
                title=f"{message.author.name}, you rolled a {data[0]} star color!",
                description=f"You currently have {stars[data[0]]} RGB {r} {g} {b}. View it on the left of this message!",
                color=discord.Color.from_rgb(int(r), int(g), int(b)),
            )
